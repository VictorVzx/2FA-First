  ...
  if (interaction.isCommand()) {
    ...
    if (commandName === "codigo_rockstar") {
      ...
      await interaction.reply({
        content:
          "Por favor, envie o **ID do canal** abaixo desta mensagem para onde você deseja enviar a mensagem.",
        ephemeral: true,
      });
    }
  }
  ...
  if (message.content) {
    ...
    if (!channel || !channel.isTextBased()) {
      return message.reply(
        "O ID fornecido não corresponde a um canal de texto válido.",
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "> Clique no botão abaixo para receber códigos da Rockstar comprados na loja. Envio instantâneo de códigos!",
      )
      .setThumbnail(client.user.displayAvatarURL())
      .setImage(
        "https://images-ext-1.discordapp.net/external/qqVVH33OE0eRml8_VIgEJ12i6LvHELyensz6nUsUaj4/%3Fw%3D1024/https/www.cnnbrasil.com.br/wp-content/uploads/sites/12/2022/09/GettyImages-1238390016-e1663593607937.jpg?format=webp&width=768&height=432",
      )
      .setTimestamp();
    ...
    try {
      ...
      await message.reply({
        content:
          "Houve um erro ao enviar a mensagem para o canal especificado.",
        ephemeral: true,
      });
  ...
  if (interaction.isButton() && interaction.customId === "get_code") {
    ...
    if (interaction.customId === "codigo_rockstar_modal") {
      ...
      const response = await interaction.reply({
        content:
          "**Por favor, espere alguns segundos enquanto estou buscando o código...**",
        ephemeral: true,
      ...
      setTimeout(async () => {
        try {
          ...
          await interaction.editReply({
            content: "Aqui está o seu código da Rockstar:",
            embeds: [embed],
          });
        } catch (error) {
          ...
          await interaction.editReply({
            content:
              "**Houve um erro ao gerar o código. Tente novamente mais tarde.**",
          });
        }
